...
<body>
  ...
  <?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($confirmation == "恶意提交") {
            $sql = "SELECT * FROM daka_records WHERE id='$id'";
            $result = mysqli_query($conn, $sql);
            $row = mysqli_fetch_array($result);
            if ($row['status'] == 0 || $row['status'] == 5) {
                // 更新数据表daka_records中的数据
                $sql = "UPDATE daka_records SET status = 4, admin = '$admin_name', admin_date = '$date', admin_time = '$time' WHERE id = '$id'";
            ...
            echo "<script>alert('输入错误！');location.href='". $_SERVER['HTTP_REFERER']. "';</script>";
        }
    }
  ?>
