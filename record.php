...
<body>
    <h1><?php echo $user['name']; ?>的打卡记录</h1>
    
<?php

    // 查询user_id为某一参数的数据并按时间排序
    $sql = "SELECT * FROM daka_records WHERE user_id = $user_id ORDER BY id ASC";
    $result = $conn->query($sql);
    
    // 输出数据
    if ($result->num_rows > 0) {
      echo "<table>";
      echo "<tr><th>打卡ID</th><th>打卡日期</th><th>打卡项目</th><th>审核状态</th><th>查看图片</th></tr>
      ";
